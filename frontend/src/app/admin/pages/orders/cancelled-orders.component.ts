  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        ...
        <p>Orders that were cancelled by customers or system</p>
      </div>
      <mat-card class="content-card">
        <mat-card-content>
          ...
          <div *ngIf="!isLoading && orders.length === 0" class="empty-state">
            ...
            <p class="subtitle">No cancelled orders at this time</p>
          </div>
...
export class CancelledOrdersComponent implements OnInit, OnDestroy {
  ...
  loadCancelledOrders(): void {
    ...
    this.orderService.getOrdersWithFallback({ 
      ...
      next: (response) => {
        console.log('Cancelled orders loaded:', response);
      ...
      error: (error) => {
        ...
        this.isLoading = false;
        this.snackBar.open('Failed to load cancelled orders', 'Close', { duration: 3000 });
      }
    });
  }
