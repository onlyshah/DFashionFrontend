...
export class RecommendationService {
  ...
  getSuggestedProducts(userId?: string, limit: number = 10): Observable<RecommendationProduct[]> {
    ...
    return this.http.get<any>(`${this.apiUrl}/api/recommendations/suggested`, { params })
      .pipe(
  ...
  getTrendingProducts(category?: string, limit: number = 10): Observable<TrendingProduct[]> {
    ...
    return this.http.get<any>(`${this.apiUrl}/api/recommendations/trending?${params.toString()}`)
      .pipe(
        map(response => response.success ? response.data : []),
        catchError(error => {
  ...
  getUserAnalytics(userId?: string): Observable<any> {
    ...
    return this.http.get<any>(`${this.apiUrl}/api/recommendations/user-analytics/${userId}`, { headers })
      .pipe(
